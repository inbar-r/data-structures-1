...
public class MeasurementsTests {
	...
	public static void main(String[] args) {
		...
		for(int i=1;i<=5;i++) {
			...
			System.out.println("when tree is balanced, and arithmetic list, avg running time is "+calAvgForInsertBalanced(new ArrayList<>(arithmetic), size)+" nano sec");
			System.out.println("when tree is not balanced, and arithmetic list, avg running time is "+calAvgForInsertUnbalanced(new ArrayList<>(arithmetic), size)+" nano sec");
			
			List<Integer> balanced = getBalancedList(size);
			System.out.println("when tree is balanced, and balanced list, avg running time is "+calAvgForInsertBalanced(new ArrayList<>(balanced), size)+" nano sec");
			System.out.println("when tree is not balanced, and balanced list, avg running time is "+calAvgForInsertUnbalanced(new ArrayList<>(balanced), size)+" nano sec");

			List<Integer> randomized = setUpRandomSet(size).stream().collect(Collectors.toList());
			System.out.println("when tree is balanced, and randomized list, avg running time is "+calAvgForInsertBalanced(new ArrayList<>(randomized), size)+" nano sec");
			System.out.println("when tree is not balanced, and randomized list, avg running time is "+calAvgForInsertUnbalanced(new ArrayList<>(randomized), size)+" nano sec");
			System.out.println("---");
		}	
	}
	...
	private static void calAndPrintTimePrefixXor(int index) {
		...
		System.out
		.println("average running time for first 100 runs of prefixXor: " + calAvg(Arrays.copyOf(times, 100))+" nano sec");
	...
	private static void calAndPrintTimeSuccPrefixXor(int index) {
		...
		System.out
		.println("average running time for first 100 runs of succPrefixXor: " + calAvg(Arrays.copyOf(times, 100))+" nano sec");
